...
if (tab in storage.tabsTime) {
  ...
  siteTodayUsage.textContent = `This Site: ${hours} ${hours === 0 ? "hr" : "hrs"} ${minutes} ${
    minutes === 0 ? "min" : "mins"
  }`;
}
...
sitesTodayUsage.textContent = `All Sites: ${hours} ${hours === 0 ? "hr" : "hrs"} ${minutes} ${
  minutes === 0 ? "min" : "mins"
}`;
...
const pomorodo = () => {
  console.log("Clicked");
  // chrome.storage.local.set({ pomorodo: true });
  // chrome.action.setPopup({ popup: "/popup/pomodoro/pomodoro.html" });
  document.location.href = "./pomodoro/pomodoro.html";
};

const timer_button = document.getElementById("timer");
timer_button.addEventListener("click", pomorodo);
